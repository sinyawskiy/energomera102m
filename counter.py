...
class Counter:
        ...
        try:
            while True:
                ...
                if _data:
                    ...
                    #print len(_buffer), _buffer[-2:], str(bytearray(self._EOL))
    ...
    def init(self, address=True):
        if address:
            init = self.readSocket(self._CMD_INIT + self.address + self._CMD_POST_INIT + self._EOL)
        else:
